...
" set linebreak						" 自定义单词断行,结合 showbreak使用
" set showbreak="->"                " 定义断行的标识符号
...
" 设置用窗口打开文件之后，窗口最下面显示文件的完整路径及文件编码,是否有bom,当前时间
if has("statusline")
set statusline=\ %{HasPaste()}%F%m%r%h\ %w\ \ CWD:\ %r%{CurDir()}/%h\ \ \ Line:\ %l/%L:%c\ \ \ \ %{\"[\".(&fenc==\"\"?&enc:&fenc).((exists(\"+bomb\")\ &&\ &bomb)?\",B\":\"\").\"]\"}\ [TYPE=%Y]\ %{strftime(\"%d/%m/%y\ -\ %H:%M\")}
endif
